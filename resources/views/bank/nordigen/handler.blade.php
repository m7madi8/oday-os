...
<script>
    ...
    const config = {
        ...
        text: "{{ ($account ?? false) && !$account->isPaid() ? config('app.name') : (isset($company) && !is_null($company) ? $company->name : config('app.name')) }} {{ ctrans('texts.nordigen_handler_subtitle', [], $lang ?? 'en') }}",
