      ...
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={query.isRefetching} onRefresh={() => query.refetch()} />}>
        ...
        {project ? (
          <>
            ...
            <Card>
              <SectionTitle title="مستندات المشروع" subtitle={`${(project.documents ?? []).length} ملف`} />
              {(project.documents ?? []).length === 0 ? <Text style={styles.meta}>لا مرفقات — ارفع من لوحة التحكم</Text> : null}
              {(project.documents ?? []).map((doc) => (
                <Row
                  key={doc.id}
                  title={doc.name || 'مستند'}
                  meta={`${categoryLabel(doc.category)} · ${doc.type || 'ملف'}`}
                />
              ))}
            </Card>
